<script setup>
import { ref, onMounted, computed } from "vue";
import { useRoute } from "vue-router";
import { useListsStore } from "../store/lists";

const store = useListsStore();
const route = useRoute();

onMounted(() => {
  store.fetchLists();
});

const tab = ref("one");
const selected = ref(null);
const search = ref(null);
const id = ref(route.query.id);

const headers = [
  { title: "Name", align: "start", key: "name" },
  { title: "Contact", align: "end", key: "contact" },
  { title: "Email", align: "end", key: "email" }
];

const listName = computed(() => {
  return store.getListById(id.value).name;
});

const listLiveStatus = computed(() => {
  return store.getListById(id.value).live;
});

const listSize = computed(() => {
  return store.getListById(id.value).contacts.length;
});

const listFilter = computed(() => {
  return store.getListById(id.value).filter;
});

const listContacts = computed(() => {
  return store.getListById(id.value).contacts;
});

const edit = () => alert("edit");
</script>

<style scoped>
.link {
  text-decoration: none;
  color: white;
}

.link:hover {
  text-decoration: underline;
}
</style>

<template>
  <v-container>
    <v-row justify="end">
      <v-col xs="12" md="10">
        <h2>{{ listName }}</h2>
      </v-col>
      <v-col xs="12" md="2">
        <v-btn variant="outlined" text="Actions"></v-btn>
      </v-col>
      <v-col xs="12" md="12" justify="center">
        <v-btn
          variant="outlined"
          size="x-small"
          class="mr-3"
          color="red"
        >{{ listLiveStatus == true ? "Live list" : "Static list" }}</v-btn>
        Size: {{ listSize }} contacts
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12">
        <v-card>
          <v-tabs v-model="tab">
            <v-tab value="one">Filters</v-tab>
          </v-tabs>
          <v-card-text>
            <v-window v-model="tab">
              <v-window-item value="one">
                <v-container>
                  <v-row>
                    <v-col xs="12" md="4" class="mt-10">{{ listFilter }}</v-col>
                    <v-col xs="12" md="8">
                      <v-container>
                        <v-row align="start">
                          <v-col cols="4">
                            <v-text-field
                              v-model="search"
                              prepend-inner-icon="mdi-magnify"
                              density="compact"
                              label="Search name, list size, description"
                              single-line
                              flat
                              hide-details
                              variant="solo-filled"
                            ></v-text-field>
                          </v-col>
                          <v-col cols="4">
                            <div v-if="selected != null && selected.length > 0">
                              <v-chip @click="edit">
                                <v-icon start icon="mdi-pencil"></v-icon>Edit
                              </v-chip>
                            </div>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-data-table
                              v-model="selected"
                              :search="search"
                              :headers="headers"
                              :items="listContacts"
                              item-value="name"
                              show-select
                              return-object
                            >
                              <template v-slot:item.name="{item}">
                                <router-link
                                  :to="`/contact?id=${item._id}`"
                                  class="link"
                                >{{ item.firstname }} {{ item.lastname }}</router-link>
                              </template>
                              <template v-slot:item.contact="{value}">
                                <a class="link" :href="`tel:+${value}`">{{ value }}</a>
                              </template>
                            </v-data-table>
                          </v-col>
                        </v-row>
                      </v-container>
                    </v-col>
                  </v-row>
                </v-container>
              </v-window-item>
            </v-window>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
</template>